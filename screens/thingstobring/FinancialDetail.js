...
const FinancialDetail = () => {
  ...
  const [financialItems, setFinancialItems] = useState([
    { id: 1, name: 'Cash (local currency and some emergency cash in home currency)', selected: false },
    { id: 2, name: 'Credit/debit cards', selected: false },
    { id: 3, name: 'Forex card', selected: false },
    { id: 4, name: 'Emergency contact information', selected: false },
  ]);
  ...
  useEffect(() => {
    const loadSelections = async () => {
      try {
        const storedItems = await AsyncStorage.getItem('selectedFinancialItems');
        if (storedItems) {
          setFinancialItems(JSON.parse(storedItems));
        }
      } catch (error) {
  ...
  const toggleCheckbox = async (id) => {
    ...
    try {
      await AsyncStorage.setItem('selectedFinancialItems', JSON.stringify(updatedItems));
    } catch (error) {
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.headerContainer}>
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>Financial Items</Text>
        </View>
      </View>
