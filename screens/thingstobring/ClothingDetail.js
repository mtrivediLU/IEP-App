...
const ClothingDetail = () => {
  ...
  const [clothingItems, setClothingItems] = useState([
    { id: 1, name: 'Seasonal clothing', selected: false },
    { id: 2, name: 'Casual clothes for everyday wear', selected: false },
    { id: 3, name: 'Formal clothes for presentations or events', selected: false },
    { id: 4, name: 'Comfortable shoes', selected: false },
    { id: 5, name: 'Easily water washable shoes - Crocs', selected: false },
    { id: 6, name: 'Sleepwear', selected: false },
    { id: 7, name: 'Undergarments and socks', selected: false },
    { id: 8, name: 'Jacket/coat', selected: false },
    { id: 9, name: 'Cap', selected: false },
  ]);
  ...
  useEffect(() => {
    const loadSelections = async () => {
      try {
        const storedItems = await AsyncStorage.getItem('selectedClothingItems');
        if (storedItems) {
  ...
  const toggleCheckbox = async (id) => {
    ...
    try {
      await AsyncStorage.setItem('selectedClothingItems', JSON.stringify(updatedItems));
    } catch (error) {
