...
const PersonalCareDetail = () => {
  ...
  const [personalCareItems, setPersonalCareItems] = useState([
    { id: 1, name: 'Toiletries (toothbrush, toothpaste, shampoo, ShowerGel, etc.)', selected: false },
    { id: 2, name: 'Skincare products', selected: false },
    { id: 3, name: 'Prescription medications (with the prescription)', selected: false },
    { id: 4, name: 'Personal hygiene products', selected: false },
    { id: 5, name: 'Small first aid kit', selected: false },
    { id: 6, name: 'Hand sanitiser', selected: false },
    { id: 8, name: 'Hair care products (comb, hair serum, etc.)', selected: false },
  ]);
  ...
  useEffect(() => {
    const loadSelections = async () => {
      try {
        const storedItems = await AsyncStorage.getItem('selectedPersonalCareItems');
        if (storedItems) {
          setPersonalCareItems(JSON.parse(storedItems));
        }
      } catch (error) {
  ...
  const toggleCheckbox = async (id) => {
    const updatedItems = personalCareItems.map((item) =>
      item.id === id ? { ...item, selected: !item.selected } : item // Toggle selection state
    );
    setPersonalCareItems(updatedItems);

    // Save the updated state to AsyncStorage
    try {
      await AsyncStorage.setItem('selectedPersonalCareItems', JSON.stringify(updatedItems));
    } catch (error) {
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.headerContainer}>
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>Personal Care</Text>
        </View>
      </View>

      {/* Personal Care Items List */}
      <FlatList
        data={personalCareItems}
